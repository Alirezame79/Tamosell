...
export default function Button({children, type, className, onClick}) {
  ...
  return (
    <>
      ...
      { type === 'secondary' && 
      <button onClick={onClick} className={`outline outline-2 outline-orange-400 py-1.5 px-3 rounded-xl bg-gray-100 hover:bg-orange-400 ${className}`}>{children}</button> }
    </>
  )
}